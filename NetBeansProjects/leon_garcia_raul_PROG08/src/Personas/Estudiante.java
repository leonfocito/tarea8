...
public class Estudiante extends Persona implements Comparable {
    ...
    /**
     * Método que comprueba mediante una expresión regular si el identificador
     * es válido.
     *
    ...
    public int compareTo(Object o) {
        int resul = 0;
        int num = Integer.parseInt(this.getIdentificador().substring(4, 10));
        String obj;
        if (o instanceof Estudiante) {
            Estudiante e = (Estudiante) o;
            obj = e.getIdentificador().substring(4, 10);
            int numO = Integer.parseInt(obj);
        if (num < numO) {
            resul = -1;
        } else if (num > numO) {
            resul = 1;
